...
const MacroTab = () => {
  ...
  const [events, setEvents] = useState<MacroEvent[]>(macroEvents);
  const [chain, setChain] = useState<CausalChainResponse | null>(null);
  const [loadingChain, setLoadingChain] = useState(false);
  const [chainError, setChainError] = useState<string | null>(null);

  useEffect(() => {
    fetchMacroEvents().then(setEvents).catch(() => setEvents(macroEvents));
  }, []);

  const loadChain = useCallback(async (eventId: string) => {
    setLoadingChain(true);
    setChainError(null);
    setChain(null);
    try {
      const data = await fetchCausalChain(eventId);
      setChain(data);
    } catch (e: unknown) {
  ...
  if (selectedEvent && (chain || loadingChain || chainError)) {
    // ── Detail View ──
    return (
      <motion.div
      ...
      >
        ...
        {loadingChain && (
          <div className="flex flex-col items-center justify-center gap-3 py-16">
            ...
            <p className="text-sm text-muted-foreground">AI is tracing the impact…</p>
          </div>
        )}
